...
public sealed class GroupService(
    ...
    IDatabaseContext context) : OnRequestServiceBase(httpContextAccessor, context)
{
    public async Task<InvokeResult<GroupModel>> CreateAsync(GroupSpec spec)
    {
        try
        {
            ...
            var group = await Group.CreateAsync(context, validationResult.CheckedResult, currentUser.CheckedResult);
    ...
    private async Task<InvokeResult<GroupInfo>> ValidateAsync(GroupInfo spec)
    {
        ...
        var currency = await context.Currencies.FindAsync(currencyName);
        if (currency is null)
